...
const char* streamfx::filter::denoising::cstring(denoising_provider provider)
{
	...
	case denoising_provider::NVIDIA_DENOISING:
		return obs_module_text(ST_I18N_PROVIDER_NVIDIA_DENOISING);
	default:
...
void streamfx::filter::denoising::denoising_instance::nvvfx_denoising_properties(obs_properties_t* pr)
{
	obs_properties_t* prg = obs_properties_create();
	obs_properties_add_group(pr, ST_KEY_NVIDIA_DENOISING, obs_module_text(ST_I18N_NVIDIA_DENOISING), OBS_GROUP_NORMAL, prg);

	{
		auto p = obs_properties_add_list(prg, ST_KEY_NVIDIA_DENOISING_STRENGTH, obs_module_text(ST_I18N_NVIDIA_DENOISING_STRENGTH), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
		obs_property_list_add_int(p, obs_module_text(ST_I18N_NVIDIA_DENOISING_STRENGTH_WEAK), 0);
		obs_property_list_add_int(p, obs_module_text(ST_I18N_NVIDIA_DENOISING_STRENGTH_STRONG), 1);
	}
}
...
obs_properties_t* denoising_factory::get_properties2(denoising_instance* data)
{
	...
	{ // Advanced Settings
...

		{
			...
			obs_property_list_add_int(p, obs_module_text(S_STATE_AUTOMATIC), static_cast<int64_t>(denoising_provider::AUTOMATIC));
			obs_property_list_add_int(p, obs_module_text(ST_I18N_PROVIDER_NVIDIA_DENOISING), static_cast<int64_t>(denoising_provider::NVIDIA_DENOISING));
		}
	}
