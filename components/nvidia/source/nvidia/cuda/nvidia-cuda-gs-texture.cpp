...
streamfx::nvidia::cuda::gstexture::gstexture(std::shared_ptr<streamfx::obs::gs::texture> texture) : _cuda(::streamfx::nvidia::cuda::cuda::get()), _texture(texture), _resource(), _is_mapped(false), _pointer()
{
	...
	case GS_DEVICE_DIRECT3D_11: {
		ID3D11Resource* resource = static_cast<ID3D11Resource*>(gs_texture_get_obj(*_texture));
		if (!resource) {
			throw std::runtime_error("nvidia::cuda::gstexture: Failed to get resource from graphics driver.");
		}
	...
	case GS_DEVICE_OPENGL: {
		GLuint* resource = static_cast<GLuint*>(gs_texture_get_obj(*_texture));
		if (!resource) {
			throw std::runtime_error("nvidia::cuda::gstexture: Failed to get resource from graphics driver.");
		}

		switch (_cuda->cuGraphicsGLRegisterImage(&_resource, *resource, GL_TEXTURE_2D, 0)) {
		case streamfx::nvidia::cuda::result::SUCCESS:
			break;
		default:
			throw std::runtime_error("nvidia::cuda::gstexture: Failed to register resource.");
