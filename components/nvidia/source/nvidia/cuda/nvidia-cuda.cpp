...
streamfx::nvidia::cuda::cuda::cuda() : _library()
{
	...
	for (auto k : names) {
		try {
			_library = streamfx::util::library::load(std::string_view(k));
		} catch (...) {
		}
	}
	if (!_library) {
		throw std::runtime_error("CUDA driver library not installed on the system.");
	}
	...
	{ // 3. Load remaining functions.
		...
		P_CUDA_LOAD_SYMBOL(cuGLGetDevices);
		P_CUDA_LOAD_SYMBOL(cuGraphicsGLRegisterBuffer);
		P_CUDA_LOAD_SYMBOL(cuGraphicsGLRegisterImage);
