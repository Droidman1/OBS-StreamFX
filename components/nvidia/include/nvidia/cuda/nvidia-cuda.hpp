...
namespace streamfx::nvidia::cuda {
	...
	enum class gl_device_list_mode : uint32_t {
		INVALID = 0x0,
		ALL     = 0x1, // Lists all devices.
		UNK2    = 0x2, // Also lists all devices.
		UNK3    = 0x3, // Also lists all devices again.
		// Do x2, x3 behave differently with multiple NVIDIA GPUs?
	};
	...
	class cuda {
		...
		P_CUDA_DEFINE_FUNCTION(cuGLGetDevices, uint32_t* count, device_t* devices, uint32_t devices_count, gl_device_list_mode mode);
		P_CUDA_DEFINE_FUNCTION(cuGraphicsGLRegisterBuffer, graphics_resource_t* resource, GLuint gl_resource, uint32_t flags);
		P_CUDA_DEFINE_FUNCTION(cuGraphicsGLRegisterImage, graphics_resource_t* resource, GLuint gl_resource, GLenum target, uint32_t flags);
