...
namespace streamfx::nvidia::cv {
	...
	struct image_t {
		...
		/** Pitch (in bytes)
		 *
		 * This is used for the X offset into `->pixels`
		 */
		int32_t        pitch;
		 ...
		 *
		 * Must be one of the enumeration values in `memory_location`.
		 */
		unsigned char mem_location;
		/** Color Information
		 * 
		 * Must be one of the enumeration values in `color_information`.
		 */
		unsigned char  color_info;
		uint8_t        reserved[2];
		void*          pixels;
		/** Value to be "deleted" when the image is deallocated.
		 *
		 * If this is set but delete_function is not, `free()` is called on this pointer.
		 */
		void* delete_pointer;
	...
	class exception : public std::runtime_error {
		...
		public:
		exception(const char* what, result code)
			: std::runtime_error(what), _code(code) {}
		exception(std::string_view what, result code)
			: std::runtime_error(what.data()), _code(code) {}
		~exception() {};
