...
obs_properties_t* transform_factory::get_properties2(transform_instance* data)
{
	...
	// Camera
	{
		...
		{ // Projection Mode
			auto p = obs_properties_add_list(prg, ST_KEY_CAMERA_MODE, obs_module_text(ST_I18N_CAMERA_MODE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_CAMERA_MODE_CORNER_PIN), static_cast<int64_t>(transform_mode::CORNER_PIN));
			obs_property_list_add_int(p, obs_module_text(ST_I18N_CAMERA_MODE_ORTHOGRAPHIC), static_cast<int64_t>(transform_mode::ORTHOGRAPHIC));
			obs_property_list_add_int(p, obs_module_text(ST_I18N_CAMERA_MODE_PERSPECTIVE), static_cast<int64_t>(transform_mode::PERSPECTIVE));
			obs_property_set_modified_callback(p, modified_camera_mode);
		}
		{ // Field Of View
			auto p = obs_properties_add_float_slider(prg, ST_KEY_CAMERA_FIELDOFVIEW, obs_module_text(ST_I18N_CAMERA_FIELDOFVIEW), 1.0, 179.0, 0.01);
		}

		obs_properties_add_group(pr, ST_I18N_CAMERA, obs_module_text(ST_I18N_CAMERA), OBS_GROUP_NORMAL, prg);
	}

	{
		...
		{ // Position
			...
			for (const auto& opt : opts) {
				auto p = obs_properties_add_float(prg, opt.first.c_str(), obs_module_text(opt.second.c_str()), std::numeric_limits<float>::lowest(), std::numeric_limits<float>::max(), 0.01);
			}

			obs_properties_add_group(pr, ST_I18N_POSITION, obs_module_text(ST_I18N_POSITION), OBS_GROUP_NORMAL, prg);
		}
		{ // Rotation
			auto prg = obs_properties_create();

			std::pair<std::string, std::string> opts[] = {
				{ST_KEY_ROTATION_X, obs_module_text(ST_I18N_ROTATION ".X")},
				{ST_KEY_ROTATION_Y, obs_module_text(ST_I18N_ROTATION ".Y")},
				{ST_KEY_ROTATION_Z, obs_module_text(ST_I18N_ROTATION ".Z")},
			};
			for (const auto& opt : opts) {
				auto p = obs_properties_add_float_slider(prg, opt.first.c_str(), obs_module_text(opt.second.c_str()), -180.0, 180.0, 0.01);
				obs_property_float_set_suffix(p, "° Deg");
			}

			obs_properties_add_group(pr, ST_I18N_ROTATION, obs_module_text(ST_I18N_ROTATION), OBS_GROUP_NORMAL, prg);
		}
		{ // Scale
			auto prg = obs_properties_create();
			...
			for (const auto& opt : opts) {
				auto p = obs_properties_add_float_slider(prg, opt.first.c_str(), opt.second.c_str(), -1000, 1000, 0.01);
				obs_property_float_set_suffix(p, "%");
			}

			obs_properties_add_group(pr, ST_I18N_SCALE, obs_module_text(ST_I18N_SCALE), OBS_GROUP_NORMAL, prg);
		}
		{ // Shear
			auto prg = obs_properties_create();
			...
			for (const auto& opt : opts) {
				auto p = obs_properties_add_float_slider(prg, opt.first.c_str(), opt.second.c_str(), -200.0, 200.0, 0.01);
				obs_property_float_set_suffix(p, "%");
			}

			obs_properties_add_group(pr, ST_I18N_SHEAR, obs_module_text(ST_I18N_SHEAR), OBS_GROUP_NORMAL, prg);
		}
	}
	...
	{ // Corners
		...
		{ // Top Left
			...
			obs_properties_add_group(prg, ST_I18N_CORNERS_TOPLEFT, obs_module_text(ST_I18N_CORNERS_TOPLEFT), OBS_GROUP_NORMAL, grp2);
		}
		{ // Top Right
			auto grp2 = obs_properties_create();
			...
			obs_properties_add_group(prg, ST_I18N_CORNERS_TOPRIGHT, obs_module_text(ST_I18N_CORNERS_TOPRIGHT), OBS_GROUP_NORMAL, grp2);
		}
		{ // Bottom Left
			auto grp2 = obs_properties_create();
			...
			obs_properties_add_group(prg, ST_I18N_CORNERS_BOTTOMLEFT, obs_module_text(ST_I18N_CORNERS_BOTTOMLEFT), OBS_GROUP_NORMAL, grp2);
		}
		{ // Bottom Right
			auto grp2 = obs_properties_create();
			...
			obs_properties_add_group(prg, ST_I18N_CORNERS_BOTTOMRIGHT, obs_module_text(ST_I18N_CORNERS_BOTTOMRIGHT), OBS_GROUP_NORMAL, grp2);
		}

		obs_properties_add_group(pr, ST_I18N_CORNERS, obs_module_text(ST_I18N_CORNERS), OBS_GROUP_NORMAL, prg);
	}

	{
		...
		{ // Mip-mapping
			auto p = obs_properties_add_bool(prg, ST_KEY_MIPMAPPING, obs_module_text(ST_I18N_MIPMAPPING));
		}

		{ // Order
			auto p = obs_properties_add_list(prg, ST_KEY_ROTATION_ORDER, obs_module_text(ST_I18N_ROTATION_ORDER), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_ROTATION_ORDER_XYZ), RotationOrder::XYZ);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_ROTATION_ORDER_XZY), RotationOrder::XZY);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_ROTATION_ORDER_YXZ), RotationOrder::YXZ);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_ROTATION_ORDER_YZX), RotationOrder::YZX);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_ROTATION_ORDER_ZXY), RotationOrder::ZXY);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_ROTATION_ORDER_ZYX), RotationOrder::ZYX);
		}
	}
...
static auto loader = streamfx::component(
	...
	{"core::gs::texrender", "core::gs::texture", "core::gs::sampler"});
