...
#define ST_I18N_THRESHOLD ST_I18N ".Threshold"
#define ST_KEY_THRESHOLD_LOW "Threshold.Low"
#define ST_I18N_THRESHOLD_LOW ST_I18N_THRESHOLD ".Low"
#define ST_KEY_THRESHOLD_HIGH "Threshold.High"
#define ST_I18N_THRESHOLD_HIGH ST_I18N_THRESHOLD ".High"
...
#define ST_KEY_NVIDIA_LATENCY ST_KEY_NVIDIA_GREENSCREEN ".Latency"
#define ST_I18N_NVIDIA_LATENCY ST_I18N_NVIDIA_GREENSCREEN ".Latency"
#endif
...
const char* streamfx::filter::virtual_greenscreen::cstring(virtual_greenscreen_provider provider)
{
	...
	case virtual_greenscreen_provider::AUTOMATIC:
		return obs_module_text(S_STATE_AUTOMATIC);
	case virtual_greenscreen_provider::NVIDIA_GREENSCREEN:
		return obs_module_text(ST_I18N_PROVIDER_NVIDIA_GREENSCREEN);
	default:
...
{
	_threshold_min = obs_data_get_double(data, ST_KEY_THRESHOLD_LOW) / 100.;
	_threshold_max = obs_data_get_double(data, ST_KEY_THRESHOLD_HIGH) / 100.;
...
void virtual_greenscreen_instance::video_render(gs_effect_t* effect)
{
	...
	{ // Draw the result for the next filter to use.
		...
		float threshold_range = _threshold_max - _threshold_min;
		float threshold_center = _threshold_min + threshold_range / 2.;
		if (_effect->has_parameter("Threshold", ::streamfx::obs::gs::effect_parameter::type::Float)) {
			_effect->get_parameter("Threshold").set_float(threshold_center);
		}
		if (_effect->has_parameter("ThresholdRange", ::streamfx::obs::gs::effect_parameter::type::Float)) {
			_effect->get_parameter("ThresholdRange").set_float(threshold_range);
		}
...
void streamfx::filter::virtual_greenscreen::virtual_greenscreen_instance::switch_provider(virtual_greenscreen_provider provider)
{
	...
	_provider_task = streamfx::util::threadpool::threadpool::instance()->push(std::bind(&virtual_greenscreen_instance::task_switch_provider, this, std::placeholders::_1), spd);
}
...
void streamfx::filter::virtual_greenscreen::virtual_greenscreen_instance::nvvfxgs_size()
{
	...
	_nvidia_fx->calculate_size(_size);
}
...
void streamfx::filter::virtual_greenscreen::virtual_greenscreen_instance::nvvfxgs_properties(obs_properties_t* prp)
{
	obs_properties_t* pr = obs_properties_create();
	obs_properties_add_group(prp, ST_KEY_NVIDIA_GREENSCREEN, obs_module_text(ST_I18N_NVIDIA_GREENSCREEN), OBS_GROUP_NORMAL, pr);

	{
		auto p = obs_properties_add_list(pr, ST_KEY_NVIDIA_GREENSCREEN_MODE, obs_module_text(ST_I18N_NVIDIA_GREENSCREEN_MODE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
		obs_property_list_add_int(p, obs_module_text(ST_I18N_NVIDIA_GREENSCREEN_MODE_PERFORMANCE), static_cast<int64_t>(::streamfx::nvidia::vfx::greenscreen_mode::PERFORMANCE));
		obs_property_list_add_int(p, obs_module_text(ST_I18N_NVIDIA_GREENSCREEN_MODE_QUALITY), static_cast<int64_t>(::streamfx::nvidia::vfx::greenscreen_mode::QUALITY));
	}
	...
	{ // Advanced Settings
		...
		{
			auto p = obs_properties_add_int_slider(prg, ST_KEY_NVIDIA_LATENCY, obs_module_text(ST_I18N_NVIDIA_LATENCY), 1, 5, 1);
...
void streamfx::filter::virtual_greenscreen::virtual_greenscreen_instance::nvvfxgs_update(obs_data_t* data)
{
	...
	_nvidia_fx->set_latency(obs_data_get_int(data, ST_KEY_NVIDIA_LATENCY));
}
...
{
	obs_data_set_default_double(data, ST_KEY_THRESHOLD_LOW, 50.);
	obs_data_set_default_double(data, ST_KEY_THRESHOLD_HIGH, 83.34);
	...
	obs_data_set_default_int(data, ST_KEY_NVIDIA_LATENCY, 1);
#endif
}
...
obs_properties_t* virtual_greenscreen_factory::get_properties2(virtual_greenscreen_instance* data)
{
	...
	{
		auto prg = obs_properties_create();
		obs_properties_add_group(pr, ST_I18N_THRESHOLD, obs_module_text(ST_I18N_THRESHOLD), OBS_GROUP_NORMAL, prg);

		{
			auto p = obs_properties_add_float_slider(prg, ST_KEY_THRESHOLD_LOW, obs_module_text(ST_I18N_THRESHOLD_LOW), 0.0, 100.0, 0.01);
			obs_property_float_set_suffix(p, "%");
			obs_property_set_modified_callback2(
				p,
				[](void* priv, obs_properties_t* props, obs_property_t* property, obs_data_t* settings) {
					auto l = obs_data_get_double(settings, ST_KEY_THRESHOLD_LOW);
					auto h = obs_data_get_double(settings, ST_KEY_THRESHOLD_HIGH);
					if (l > h) {
						obs_data_set_double(settings, ST_KEY_THRESHOLD_HIGH, l);
						return true;
					}
					return false;
				},
		...
		{
			auto p = obs_properties_add_float_slider(prg, ST_KEY_THRESHOLD_HIGH, obs_module_text(ST_I18N_THRESHOLD_HIGH), 0.0, 100.0, 0.01);
			obs_property_float_set_suffix(p, "%");
			obs_property_set_modified_callback2(
				p,
				[](void* priv, obs_properties_t* props, obs_property_t* property, obs_data_t* settings) {
					...
					auto h = obs_data_get_double(settings, ST_KEY_THRESHOLD_HIGH);
					if (h < l) {
						obs_data_set_double(settings, ST_KEY_THRESHOLD_LOW, h);
						return true;
					}
					return false;
				},
	...
	{ // Advanced Settings
		...
		{
			auto p = obs_properties_add_list(prg, ST_KEY_PROVIDER, obs_module_text(ST_I18N_PROVIDER), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
			obs_property_set_modified_callback(p, modified_provider);
			obs_property_list_add_int(p, obs_module_text(S_STATE_AUTOMATIC), static_cast<int64_t>(virtual_greenscreen_provider::AUTOMATIC));
			obs_property_list_add_int(p, obs_module_text(ST_I18N_PROVIDER_NVIDIA_GREENSCREEN), static_cast<int64_t>(virtual_greenscreen_provider::NVIDIA_GREENSCREEN));
		}
	}
