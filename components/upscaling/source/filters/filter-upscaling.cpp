...
const char* streamfx::filter::upscaling::cstring(upscaling_provider provider)
{
	...
	case upscaling_provider::NVIDIA_SUPERRESOLUTION:
		return obs_module_text(ST_I18N_PROVIDER_NVIDIA_SUPERRES);
	default:
...
void streamfx::filter::upscaling::upscaling_instance::nvvfxsr_properties(obs_properties_t* props)
{
	...
	obs_properties_add_group(props, ST_KEY_NVIDIA_SUPERRES, obs_module_text(ST_I18N_NVIDIA_SUPERRES), OBS_GROUP_NORMAL, grp);

	{
		auto p = obs_properties_add_list(grp, ST_KEY_NVIDIA_SUPERRES_STRENGTH, obs_module_text(ST_I18N_NVIDIA_SUPERRES_STRENGTH), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
		obs_property_list_add_int(p, obs_module_text(ST_I18N_NVIDIA_SUPERRES_STRENGTH_WEAK), 0);
		obs_property_list_add_int(p, obs_module_text(ST_I18N_NVIDIA_SUPERRES_STRENGTH_STRONG), 1);
	}

	{
		auto p = obs_properties_add_float_slider(grp, ST_KEY_NVIDIA_SUPERRES_SCALE, obs_module_text(ST_I18N_NVIDIA_SUPERRES_SCALE), 100.00, 400.00, .01);
...
obs_properties_t* upscaling_factory::get_properties2(upscaling_instance* data)
{
	...
	{ // Advanced Settings
...

		{
			...
			obs_property_list_add_int(p, obs_module_text(S_STATE_AUTOMATIC), static_cast<int64_t>(upscaling_provider::AUTOMATIC));
			obs_property_list_add_int(p, obs_module_text(ST_I18N_PROVIDER_NVIDIA_SUPERRES), static_cast<int64_t>(upscaling_provider::NVIDIA_SUPERRESOLUTION));
		}
	}
