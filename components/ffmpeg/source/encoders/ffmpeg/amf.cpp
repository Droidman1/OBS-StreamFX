...
void amf::properties_before(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props, AVCodecContext* context)
{
	...
	{ // Quality, but not the one from FFmpeg.
		...
		auto p = obs_properties_add_list(props, ST_KEY_QUALITY, obs_module_text(ST_I18N_QUALITY), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
		/* streamfx::ffmpeg::tools::avoption_list_add_entries(context->priv_data, "quality", [&p](const AVOption* opt) {
			...
			obs_property_list_add_string(p, streamfx::translate(buffer, opt->name), opt->name);
		});*/
		for (auto key : {"speed", "balanced", "quality", "high_quality"}) {
			char buffer[1024];
			snprintf(buffer, sizeof(buffer), "%s.%s", ST_I18N_QUALITY, key);
			obs_property_list_add_string(p, streamfx::translate(buffer, key), key);
		}
	}

	{ // Usage, but again not from FFmpeg.
		// We ignore the FFmpeg one here, since AMD is - as usual - inconsistent and not backwards compatible.
		auto p = obs_properties_add_list(props, ST_KEY_USAGE, obs_module_text(ST_I18N_USAGE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
...
void amf::properties_after(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props, AVCodecContext* context)
{
	...
	{ // Rate Control
		...
		{ // Mode
			...
			streamfx::ffmpeg::tools::avoption_list_add_entries(context->priv_data, "rc", [&p](const AVOption* opt) {
				...
				obs_property_list_add_string(p, streamfx::translate(buffer, opt->help ? opt->help : opt->name), opt->name);
			});
		}

		if (avoption_exists(context->priv_data, "preencode")) { // Pre-Encode
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_RATECONTROL_PREENCODE, obs_module_text(ST_I18N_RATECONTROL_PREENCODE));
		}

		{ // Limits
			obs_properties_t* grp2 = obs_properties_create();
			obs_properties_add_group(grp, ST_I18N_RATECONTROL_LIMITS, obs_module_text(ST_I18N_RATECONTROL_LIMITS), OBS_GROUP_NORMAL, grp2);

			if (avoption_exists(context->priv_data, "qvbr_quality_level")) { // Quality
				auto p = obs_properties_add_int_slider(grp2, ST_KEY_RATECONTROL_LIMITS_QUALITY, obs_module_text(ST_I18N_RATECONTROL_LIMITS_QUALITY), -1, 51, 1);
			}

			{ // Bitrate Target
				auto p = obs_properties_add_int(grp2, ST_KEY_RATECONTROL_LIMITS_BITRATE_TARGET, obs_module_text(ST_I18N_RATECONTROL_LIMITS_BITRATE_TARGET), -1, std::numeric_limits<int32_t>::max(), 1);
				obs_property_int_set_suffix(p, " kbit/s");
			}

			{ // Bitrate Maximum
				auto p = obs_properties_add_int(grp2, ST_KEY_RATECONTROL_LIMITS_BITRATE_MAXIMUM, obs_module_text(ST_I18N_RATECONTROL_LIMITS_BITRATE_MAXIMUM), -1, std::numeric_limits<int32_t>::max(), 1);
				obs_property_int_set_suffix(p, " kbit/s");
			}

			{ // Buffer Size
				auto p = obs_properties_add_int(grp2, ST_KEY_RATECONTROL_LIMITS_BUFFERSIZE, obs_module_text(ST_I18N_RATECONTROL_LIMITS_BUFFERSIZE), 0, std::numeric_limits<int32_t>::max(), 1);
		...
		{ // QP
			obs_properties_t* grp2 = obs_properties_create();
			obs_properties_add_group(grp, ST_I18N_RATECONTROL_QP, obs_module_text(ST_I18N_RATECONTROL_QP), OBS_GROUP_NORMAL, grp2);

			if (avoption_exists(context->priv_data, "qp_i")) {
				auto p = obs_properties_add_int_slider(grp2, ST_KEY_RATECONTROL_QP_I, obs_module_text(ST_I18N_RATECONTROL_QP_I), -1, 51, 1);
			}
			if (avoption_exists(context->priv_data, "qp_p")) {
				auto p = obs_properties_add_int_slider(grp2, ST_KEY_RATECONTROL_QP_P, obs_module_text(ST_I18N_RATECONTROL_QP_P), -1, 51, 1);
			}
			if (avoption_exists(context->priv_data, "qp_b")) {
				auto p = obs_properties_add_int_slider(grp2, ST_KEY_RATECONTROL_QP_B, obs_module_text(ST_I18N_RATECONTROL_QP_B), -1, 51, 1);
			}
		}
	}
	...
	if (avoption_exists(context->priv_data, "preanalysis")) { // Pre-Analysis
		obs_properties_t* grp = obs_properties_create();
		obs_properties_add_group(props, ST_I18N_PA, obs_module_text(ST_I18N_PA), OBS_GROUP_NORMAL, grp);

		{ // Pre-Analysis
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_PA, obs_module_text(ST_I18N_PA));
		...
		if (avoption_exists(context->priv_data, "pa_lookahead_buffer_depth")) { // Look-Ahead
			auto p = obs_properties_add_int_slider(grp, ST_KEY_PA_LOOKAHEAD, obs_module_text(ST_I18N_PA_LOOKAHEAD), -1, 41, 1);
			obs_property_int_set_suffix(p, " frames");
		}

		if (avoption_exists(context->priv_data, "pa_scene_change_detection_enable")) { // Scene Change Detection
			auto p = obs_properties_add_list(grp, ST_KEY_PA_SCENECHANGEDETECTION, obs_module_text(ST_I18N_PA_SCENECHANGEDETECTION), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "default");
			obs_property_list_add_string(p, obs_module_text(S_STATE_DISABLED), "disabled");
		...
		if (avoption_exists(context->priv_data, "pa_static_scene_detection_enable")) { // Static Scene Detection
			auto p = obs_properties_add_list(grp, ST_KEY_PA_STATICSCENEDETECTION, obs_module_text(ST_I18N_PA_STATICSCENEDETECTION), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "default");
			obs_property_list_add_string(p, obs_module_text(S_STATE_DISABLED), "disabled");
		...
		if (avoption_exists(context->priv_data, "pa_paq_mode") || avoption_exists(context->priv_data, "pa_taq_mode")) { // Adaptive Quantization
			obs_properties_t* grp2 = obs_properties_create();
			obs_properties_add_group(grp, ST_I18N_PA_AQ, obs_module_text(ST_I18N_PA_AQ), OBS_GROUP_NORMAL, grp2);

			if (avoption_exists(context->priv_data, "pa_paq_mode")) { // Perceptual Adaptive Quantization
				auto p = obs_properties_add_list(grp2, ST_KEY_PA_AQ_PERCEPTUAL, obs_module_text(ST_I18N_PA_AQ_PERCEPTUAL), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
				...
				if (avoption_exists(context->priv_data, "pa_caq_strength")) { // Content Adaptive Quantization Strength
					auto p = obs_properties_add_list(grp2, ST_KEY_PA_AQ_CONTENT, obs_module_text(ST_I18N_PA_AQ_CONTENT), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
					obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "default");
			...
			if (avoption_exists(context->priv_data, "pa_taq_mode")) { // Temporal Adaptive Quantization
				auto p = obs_properties_add_list(grp2, ST_KEY_PA_AQ_TEMPORAL, obs_module_text(ST_I18N_PA_AQ_TEMPORAL), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
				obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "default");
		...
		if (avoption_exists(context->priv_data, "pa_high_motion_quality_boost_mode")) { // High Motion Quality Boost
			auto p = obs_properties_add_list(grp, ST_KEY_PA_HIGHMOTIONQUALITYBOOST, obs_module_text(ST_I18N_PA_HIGHMOTIONQUALITYBOOST), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "default");
	...
	{ // Other Options
		...
		if (std::string_view{"h264_amf"} == codec->name) { // Only H264
			{
				auto p = obs_properties_add_int_slider(grp, ST_KEY_OTHER_BFRAMES, obs_module_text(ST_I18N_OTHER_BFRAMES), -1, 3, 1);
			}

			{
				auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_OTHER_BFRAMEREFERENCES, obs_module_text(ST_I18N_OTHER_BFRAMEREFERENCES));
			}
		}
		...
		if (avoption_exists(context->priv_data, "vbaq")) { // Not AV1
			{
				auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_OTHER_VBAQ, obs_module_text(ST_I18N_OTHER_VBAQ));
			}
		}

		// High Motion Quality Boost
		if (avoption_exists(context->priv_data, "high_motion_quality_boost_enable")) {
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_OTHER_HIGHMOTIONQUALITYBOOST, obs_module_text(ST_I18N_OTHER_HIGHMOTIONQUALITYBOOST));
		}

		// Maximum Reference Frames (16 for non-AV1, 8 for AV1)
		if (std::string_view{"av1_amf"} != codec->name) { // Not AV1
			auto p = obs_properties_add_int_slider(grp, ST_KEY_OTHER_REFERENCEFRAMES, obs_module_text(ST_I18N_OTHER_REFERENCEFRAMES), -1, 16, 1);
		} else {
			auto p = obs_properties_add_int_slider(grp, ST_KEY_OTHER_REFERENCEFRAMES, obs_module_text(ST_I18N_OTHER_REFERENCEFRAMES), -1, 8, 1);
		}
	}
}
		...
		}
		{
			...
			streamfx::ffmpeg::tools::avoption_list_add_entries(context->priv_data, "level", [&p](const AVOption* opt) {
				if (std::string_view{"auto"} == opt->name) {
					obs_property_list_add_string(p, obs_module_text(S_STATE_AUTOMATIC), opt->name);
				} else {
