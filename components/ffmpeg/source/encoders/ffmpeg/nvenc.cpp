...
void nvenc::properties_before(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props, AVCodecContext* context)
{
	...
	{
		auto p = obs_properties_add_list(props, ST_KEY_PRESET, obs_module_text(ST_I18N_PRESET), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
		streamfx::ffmpeg::tools::avoption_list_add_entries(context->priv_data, "preset", [&p](const AVOption* opt) {
			...
			obs_property_list_add_string(p, obs_module_text(buffer), opt->name);
		});
	}

	if (streamfx::ffmpeg::tools::avoption_exists(context->priv_data, "tune")) {
		auto p = obs_properties_add_list(props, ST_KEY_TUNE, obs_module_text(ST_I18N_TUNE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
...
void nvenc::properies_after(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props, AVCodecContext* context)
{
	...
	{ // Rate Control
		...
		if (!streamfx::util::are_property_groups_broken()) {
			...
			obs_properties_add_group(props, ST_I18N_RATECONTROL, obs_module_text(ST_I18N_RATECONTROL), OBS_GROUP_NORMAL, grp);
		}

		{
			auto p = obs_properties_add_list(grp, ST_KEY_RATECONTROL_MODE, obs_module_text(ST_I18N_RATECONTROL_MODE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_set_modified_callback(p, modified_ratecontrol);
			obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "");
		...
		if (streamfx::ffmpeg::tools::avoption_exists(context->priv_data, "multipass")) {
			auto p = obs_properties_add_list(grp, ST_KEY_RATECONTROL_MULTIPASS, obs_module_text(ST_I18N_RATECONTROL_MULTIPASS), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "");
			...
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_RATECONTROL_TWOPASS, obs_module_text(ST_I18N_RATECONTROL_TWOPASS));
		}

		{
			auto p = obs_properties_add_int_slider(grp, ST_KEY_RATECONTROL_LOOKAHEAD, obs_module_text(ST_I18N_RATECONTROL_LOOKAHEAD), -1, 32, 1);
			obs_property_int_set_suffix(p, " frames");
			//obs_property_set_modified_callback(p, modified_lookahead);
		}

		{
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_RATECONTROL_ADAPTIVEI, obs_module_text(ST_I18N_RATECONTROL_ADAPTIVEI));
		}

		// Unavailable with hevc_nvenc
		if (std::string_view{"hevc_nvenc"} != codec->name) {
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_RATECONTROL_ADAPTIVEB, obs_module_text(ST_I18N_RATECONTROL_ADAPTIVEB));
		}
	}

	{
		...
		if (!streamfx::util::are_property_groups_broken()) {
			grp = obs_properties_create();
			obs_properties_add_group(props, ST_I18N_RATECONTROL_LIMITS, obs_module_text(ST_I18N_RATECONTROL_LIMITS), OBS_GROUP_NORMAL, grp);
		}

		{
			auto p = obs_properties_add_float_slider(grp, ST_KEY_RATECONTROL_LIMITS_QUALITY, obs_module_text(ST_I18N_RATECONTROL_LIMITS_QUALITY), 0, 51, 0.01);
		}

		{
			auto p = obs_properties_add_int(grp, ST_KEY_RATECONTROL_LIMITS_BITRATE_TARGET, obs_module_text(ST_I18N_RATECONTROL_LIMITS_BITRATE_TARGET), -1, std::numeric_limits<int32_t>::max(), 1);
			obs_property_int_set_suffix(p, " kbit/s");
		}

		{
			auto p = obs_properties_add_int(grp, ST_KEY_RATECONTROL_LIMITS_BITRATE_MAXIMUM, obs_module_text(ST_I18N_RATECONTROL_LIMITS_BITRATE_MAXIMUM), -1, std::numeric_limits<int32_t>::max(), 1);
			obs_property_int_set_suffix(p, " kbit/s");
		}

		{
			auto p = obs_properties_add_int(grp, ST_KEY_RATECONTROL_LIMITS_BUFFERSIZE, obs_module_text(ST_I18N_RATECONTROL_LIMITS_BUFFERSIZE), 0, std::numeric_limits<int32_t>::max(), 1);
...

	{
		...
		if (!streamfx::util::are_property_groups_broken()) {
			grp = obs_properties_create();
			obs_properties_add_group(props, ST_I18N_RATECONTROL_QP, obs_module_text(ST_I18N_RATECONTROL_QP), OBS_GROUP_NORMAL, grp);
		}

		{
			auto p = obs_properties_add_int_slider(grp, ST_KEY_RATECONTROL_QP_I, obs_module_text(ST_I18N_RATECONTROL_QP_I), -1, 51, 1);
		}
		{
			auto p = obs_properties_add_int_slider(grp, ST_KEY_RATECONTROL_QP_P, obs_module_text(ST_I18N_RATECONTROL_QP_P), -1, 51, 1);
		}
		{
			auto p = obs_properties_add_int_slider(grp, ST_KEY_RATECONTROL_QP_B, obs_module_text(ST_I18N_RATECONTROL_QP_B), -1, 51, 1);
		}
	}

	{
		...
		if (!streamfx::util::are_property_groups_broken()) {
			grp = obs_properties_create();
			obs_properties_add_group(props, ST_I18N_AQ, obs_module_text(ST_I18N_AQ), OBS_GROUP_NORMAL, grp);
		}

		{
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_AQ_SPATIAL, obs_module_text(ST_I18N_AQ_SPATIAL));
			obs_property_set_modified_callback(p, modified_aq);
		}
		{
			auto p = obs_properties_add_int_slider(grp, ST_KEY_AQ_STRENGTH, obs_module_text(ST_I18N_AQ_STRENGTH), -1, 15, 1);
		}
		{
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_AQ_TEMPORAL, obs_module_text(ST_I18N_AQ_TEMPORAL));
		}
	}

	{
		...
		if (!streamfx::util::are_property_groups_broken()) {
			grp = obs_properties_create();
			obs_properties_add_group(props, ST_I18N_OTHER, obs_module_text(ST_I18N_OTHER), OBS_GROUP_NORMAL, grp);
		}

		{
			auto p = obs_properties_add_int_slider(grp, ST_KEY_OTHER_BFRAMES, obs_module_text(ST_I18N_OTHER_BFRAMES), -1, 4, 1);
			obs_property_int_set_suffix(p, " frames");
		}

		{
			auto p = obs_properties_add_list(grp, ST_KEY_OTHER_BFRAMEREFERENCEMODE, obs_module_text(ST_I18N_OTHER_BFRAMEREFERENCEMODE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "");
		...
		{
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_OTHER_ZEROLATENCY, obs_module_text(ST_I18N_OTHER_ZEROLATENCY));
		}

		{
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_OTHER_WEIGHTEDPREDICTION, obs_module_text(ST_I18N_OTHER_WEIGHTEDPREDICTION));
		}

		{
			auto p = streamfx::util::obs_properties_add_tristate(grp, ST_KEY_OTHER_NONREFERENCEPFRAMES, obs_module_text(ST_I18N_OTHER_NONREFERENCEPFRAMES));
		}

		{
			auto p = obs_properties_add_int_slider(grp, ST_KEY_OTHER_REFERENCEFRAMES, obs_module_text(ST_I18N_OTHER_REFERENCEFRAMES), -1, (strcmp(codec->name, "h264_nvenc") == 0) ? 16 : 4, 1);
			obs_property_int_set_suffix(p, " frames");
		}

		if (streamfx::ffmpeg::tools::avoption_exists(context->priv_data, "ldkfs")) {
			auto p = obs_properties_add_int_slider(grp, ST_KEY_OTHER_LOWDELAYKEYFRAMESCALE, obs_module_text(ST_I18N_OTHER_LOWDELAYKEYFRAMESCALE), -1, 255, 1);
		}
	}
}
...
#define COPY_UNSET(TYPE, FROM, TO)                                              \
	...
	if (version <= STREAMFX_MAKE_VERSION(0, 8, 0, 0)) {
		...
		COPY_UNSET(int, "RateControl.Quality.Minimum", "RateControl.QP.Minimum");
		COPY_UNSET(int, "RateControl.Quality.Maximum", "RateControl.QP.Maximum");
...
#ifndef _DEBUG
	if ((obs_get_encoder_caps("jim_nvenc") == 0) || (!nvenc::is_available())) {
		// If NVENC is not present, or the default encoder is not present, don't list ours either.
...
void nvenc_h264::properties_encoder(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props)
{
	...
	nvenc::properties_before(factory, instance, props, context.get());

	{
		obs_properties_t* grp = props;
		if (!streamfx::util::are_property_groups_broken()) {
			grp = obs_properties_create();
			obs_properties_add_group(props, S_CODEC_H264, obs_module_text(S_CODEC_H264), OBS_GROUP_NORMAL, grp);
		}

		{
			auto p = obs_properties_add_list(grp, ST_KEY_H264_PROFILE, obs_module_text(S_CODEC_H264_PROFILE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_string(p, obs_module_text(S_STATE_DEFAULT), "");
		...
		{
			auto p = obs_properties_add_list(grp, ST_KEY_H264_LEVEL, obs_module_text(S_CODEC_H264_LEVEL), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);

			streamfx::ffmpeg::tools::avoption_list_add_entries(context->priv_data, "level", [&p](const AVOption* opt) {
				if (opt->default_val.i64 == 0) {
					obs_property_list_add_string(p, obs_module_text(S_STATE_AUTOMATIC), "auto");
				} else {
	...
	nvenc::properies_after(factory, instance, props, context.get());
}
...
void nvenc_hevc::adjust_info(ffmpeg_factory* factory, std::string& id, std::string& name, std::string& codec)
{
	...
	factory->get_info()->caps |= OBS_ENCODER_CAP_DYN_BITRATE;
#ifndef _DEBUG
	if ((obs_get_encoder_caps("jim_hevc_nvenc") == 0) || (!nvenc::is_available())) {
		// If NVENC is not present, or the default encoder is not present, don't list ours either.
		factory->get_info()->caps |= OBS_ENCODER_CAP_DEPRECATED | OBS_ENCODER_CAP_INTERNAL;
	}
#endif // _DEBUG
...
void nvenc_hevc::migrate(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_data_t* settings, uint64_t version)
{
	...
	// - no other version.
	if ((version < STREAMFX_MAKE_VERSION(0, 12, 0, 0)) || ((version > STREAMFX_MAKE_VERSION(0, 12, 0, 0)) && (version < STREAMFX_MAKE_VERSION(0, 12, 0, 315)))) {
...
void nvenc_hevc::properties_encoder(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props)
{
...

	{
		...
		if (!streamfx::util::are_property_groups_broken()) {
			grp = obs_properties_create();
			obs_properties_add_group(props, S_CODEC_HEVC, obs_module_text(S_CODEC_HEVC), OBS_GROUP_NORMAL, grp);
		}

		{
			auto p = obs_properties_add_list(grp, ST_KEY_H265_PROFILE, obs_module_text(S_CODEC_HEVC_PROFILE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
		...
		{
			auto p = obs_properties_add_list(grp, ST_KEY_H265_TIER, obs_module_text(S_CODEC_HEVC_TIER), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_int(p, obs_module_text(S_STATE_DEFAULT), -1);
		...
		{
			auto p = obs_properties_add_list(grp, ST_KEY_H265_LEVEL, obs_module_text(S_CODEC_HEVC_LEVEL), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
...
nvenc_av1::nvenc_av1() : handler("av1_nvenc"){};

nvenc_av1::~nvenc_av1(){};

bool nvenc_av1::has_keyframes(ffmpeg_factory*)
{
	return true;
}

bool nvenc_av1::has_threading(ffmpeg_factory* instance)
{
	return false;
}

bool nvenc_av1::is_hardware(ffmpeg_factory* instance)
{
	return true;
}

bool nvenc_av1::is_reconfigurable(ffmpeg_factory* instance, bool& threads, bool& gpu, bool& keyframes)
{
	threads   = false;
	gpu       = false;
	keyframes = false;
...
void nvenc_av1::adjust_info(ffmpeg_factory* factory, std::string& id, std::string& name, std::string& codec)
{
	name = "NVIDIA NVENC AV1 (via FFmpeg)";
	factory->get_info()->caps |= OBS_ENCODER_CAP_DYN_BITRATE;
#ifndef _DEBUG
	if ((obs_get_encoder_caps("jim_av1_nvenc") == 0) || (!nvenc::is_available())) {
		// If NVENC is not present, or the default encoder is not present, don't list ours either.
		factory->get_info()->caps |= OBS_ENCODER_CAP_DEPRECATED | OBS_ENCODER_CAP_INTERNAL;
	}
#endif // _DEBUG
...
void nvenc_av1::defaults(ffmpeg_factory* factory, obs_data_t* settings)
{
	nvenc::defaults(factory, settings);

	obs_data_set_default_string(settings, ST_KEY_AV1_PROFILE, "");
	obs_data_set_default_string(settings, ST_KEY_AV1_LEVEL, "auto");
}

void nvenc_av1::properties(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props)
{
	if (!instance) {
		this->properties_encoder(factory, instance, props);
	} else {
...
void nvenc_av1::migrate(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_data_t* settings, uint64_t version)
{
	nvenc::migrate(factory, instance, settings, version);
}

void nvenc_av1::update(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_data_t* settings)
{
	auto codec   = factory->get_avcodec();
	auto context = instance->get_avcodeccontext();
	...
	if (!context->internal) {
		if (const char* v = obs_data_get_string(settings, ST_KEY_AV1_PROFILE); v && (v[0] != '\0')) {
			av_opt_set(context->priv_data, "tier", v, AV_OPT_SEARCH_CHILDREN);
		}
		if (const char* v = obs_data_get_string(settings, ST_KEY_AV1_LEVEL); v && (v[0] != '\0')) {
			av_opt_set(context->priv_data, "level", v, AV_OPT_SEARCH_CHILDREN);
		}
	}
}

void nvenc_av1::override_update(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_data_t* settings)
{
	nvenc::override_update(factory, instance, settings);
}

void nvenc_av1::properties_encoder(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props)
{
	std::shared_ptr<AVCodecContext> context{avcodec_alloc_context3(factory->get_avcodec()), [](AVCodecContext* ptr) { avcodec_free_context(&ptr); }};
	nvenc::properties_before(factory, instance, props, context.get());

	{
		...
		if (!streamfx::util::are_property_groups_broken()) {
			grp = obs_properties_create();
			obs_properties_add_group(props, S_CODEC_AV1, obs_module_text(S_CODEC_AV1), OBS_GROUP_NORMAL, grp);
		}

		{
			auto p = obs_properties_add_list(grp, ST_KEY_AV1_PROFILE, obs_module_text(S_CODEC_AV1_PROFILE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_int(p, obs_module_text(S_STATE_DEFAULT), -1);
			streamfx::ffmpeg::tools::avoption_list_add_entries(context->priv_data, "tier", [&p](const AVOption* opt) {
				char buffer[1024];
		...
		{
			auto p = obs_properties_add_list(grp, ST_KEY_AV1_LEVEL, obs_module_text(S_CODEC_AV1_LEVEL), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
...
void nvenc_av1::properties_runtime(ffmpeg_factory* factory, ffmpeg_instance* instance, obs_properties_t* props)
{
	nvenc::properties_runtime(factory, instance, props);
}

static auto inst_av1 = nvenc_av1();
