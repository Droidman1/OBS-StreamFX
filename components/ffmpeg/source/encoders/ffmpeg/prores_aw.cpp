...
inline const char* profile_to_name(const AVProfile* ptr)
{
	...
	case profile::APCO:
		return obs_module_text(S_CODEC_PRORES_PROFILE_APCO);
	case profile::APCS:
		return obs_module_text(S_CODEC_PRORES_PROFILE_APCS);
	case profile::APCN:
		return obs_module_text(S_CODEC_PRORES_PROFILE_APCN);
	case profile::APCH:
		return obs_module_text(S_CODEC_PRORES_PROFILE_APCH);
	case profile::AP4H:
		return obs_module_text(S_CODEC_PRORES_PROFILE_AP4H);
	case profile::AP4X:
		return obs_module_text(S_CODEC_PRORES_PROFILE_AP4X);
	default:
		return ptr->name;
	}
}
...
{
	if (!instance) {
		auto p = obs_properties_add_list(props, S_CODEC_PRORES_PROFILE, obs_module_text(S_CODEC_PRORES_PROFILE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
