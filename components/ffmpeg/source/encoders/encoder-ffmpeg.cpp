...
ffmpeg_factory::ffmpeg_factory(ffmpeg_manager* manager, const AVCodec* codec) : _avcodec(codec)
{
	...
	{ // Generate default name.
		...
		str << obs_module_text(ST_I18N_FFMPEG_SUFFIX);
		_name = str.str();
	}
...
obs_properties_t* ffmpeg_factory::get_properties2(instance_t* data)
{
...

	{
		...
		obs_property_button_set_url(p, const_cast<char*>(this->_handler->help(this).c_str()));
	}

	if (data) {
		data->get_properties(pr);
	}

	if (_handler)
		_handler->properties(this, data, pr);

	if (_handler && _handler->has_keyframes(this)) {
		// Key-Frame Options
		obs_properties_t* prg = obs_properties_create();
		obs_properties_add_group(pr, ST_I18N_KEYFRAMES, obs_module_text(ST_I18N_KEYFRAMES), OBS_GROUP_NORMAL, prg);

		{ // Key-Frame Interval Type
			auto p = obs_properties_add_list(prg, ST_KEY_KEYFRAMES_INTERVALTYPE, obs_module_text(ST_I18N_KEYFRAMES_INTERVALTYPE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
			obs_property_set_modified_callback(p, modified_keyframes);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_KEYFRAMES_INTERVALTYPE_("Seconds")), 0);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_KEYFRAMES_INTERVALTYPE_("Frames")), 1);
		}
		{ // Key-Frame Interval Seconds
			auto p = obs_properties_add_float(prg, ST_KEY_KEYFRAMES_INTERVAL_SECONDS, obs_module_text(ST_I18N_KEYFRAMES_INTERVAL), 0.00, std::numeric_limits<int16_t>::max(), 0.01);
			obs_property_float_set_suffix(p, " seconds");
		}
		{ // Key-Frame Interval Frames
			auto p = obs_properties_add_int(prg, ST_KEY_KEYFRAMES_INTERVAL_FRAMES, obs_module_text(ST_I18N_KEYFRAMES_INTERVAL), 0, std::numeric_limits<int32_t>::max(), 1);
	...
	{
		obs_properties_t* prg = obs_properties_create();
		obs_properties_add_group(pr, ST_I18N_FFMPEG, obs_module_text(ST_I18N_FFMPEG), OBS_GROUP_NORMAL, prg);

		{ // Custom Settings
			auto p = obs_properties_add_text(prg, ST_KEY_FFMPEG_CUSTOMSETTINGS, obs_module_text(ST_I18N_FFMPEG_CUSTOMSETTINGS), obs_text_type::OBS_TEXT_DEFAULT);
		}

		if (_handler && _handler->is_hardware(this)) {
			auto p = obs_properties_add_int(prg, ST_KEY_FFMPEG_GPU, obs_module_text(ST_I18N_FFMPEG_GPU), -1, std::numeric_limits<uint8_t>::max(), 1);
		}

		if (_handler && _handler->has_threading(this)) {
			auto p = obs_properties_add_int_slider(prg, ST_KEY_FFMPEG_THREADS, obs_module_text(ST_I18N_FFMPEG_THREADS), 0, static_cast<int64_t>(std::thread::hardware_concurrency()) * 2, 1);
		}
		...
		{ // Frame Skipping
			...
			auto p = obs_properties_add_list(prg, ST_KEY_FFMPEG_FRAMERATE, obs_module_text(ST_I18N_FFMPEG_FRAMERATE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
