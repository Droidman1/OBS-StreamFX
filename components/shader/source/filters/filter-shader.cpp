...
void shader_instance::video_tick(float sec_since_last)
{
	...
	if (obs_source_t* tgt = obs_filter_get_target(_self); tgt != nullptr) {
		_fx->set_base_size(obs_source_get_base_width(tgt), obs_source_get_base_height(tgt));
	} else if (obs_source* src = obs_filter_get_parent(_self); src != nullptr) {
		_fx->set_base_size(obs_source_get_base_width(src), obs_source_get_base_height(src));
	}
}
...
void shader_instance::video_render(gs_effect_t* effect)
{
	try {
		...
		streamfx::obs::gs::marker gm00(streamfx::util::format("Shader Filter: %s", obs_source_get_uuid(_self)), streamfx::obs::gs::marker::paper);
