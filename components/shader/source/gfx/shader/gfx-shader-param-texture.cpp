...
void streamfx::gfx::shader::texture_parameter::defaults(obs_data_t* settings)
{
	if (field_type() == texture_field_type::Input) {
		...
		obs_data_set_default_string(settings, _keys[1].c_str(), reinterpret_cast<const char*>(_default.generic_string().c_str()));
...
void streamfx::gfx::shader::texture_parameter::properties(obs_properties_t* props, obs_data_t* settings)
{
	...
	if (field_type() == texture_field_type::Enum) {
		...
		for (auto v : _values) {
			obs_property_list_add_string(p, v.name.c_str(), reinterpret_cast<const char*>(v.data.file.generic_string().c_str()));
		}
	} else {
		...
		{
			auto p = obs_properties_add_list(pr, _keys[0].c_str(), obs_module_text(ST_I18N_TYPE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
			obs_property_set_modified_callback2(p, modified_type, this);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_TYPE_FILE), static_cast<int64_t>(texture_type::File));
			obs_property_list_add_int(p, obs_module_text(ST_I18N_TYPE_SOURCE), static_cast<int64_t>(texture_type::Source));
		}

		{
			// ToDo: Filter and Default Path.
			auto p = obs_properties_add_path(pr, _keys[1].c_str(), obs_module_text(ST_I18N_FILE), OBS_PATH_FILE, "* (*.*)", nullptr);
		}

		{
			auto p = obs_properties_add_list(pr, _keys[2].c_str(), obs_module_text(ST_I18N_SOURCE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
			obs_property_list_add_string(p, "", "");
			obs::source_tracker::instance()->enumerate(
				[&p](std::string name, ::streamfx::obs::source) {
					std::stringstream sstr;
					sstr << name << " (" << obs_module_text(S_SOURCETYPE_SOURCE) << ")";
			...
			obs::source_tracker::instance()->enumerate(
				[&p](std::string name, ::streamfx::obs::source) {
					std::stringstream sstr;
					sstr << name << " (" << obs_module_text(S_SOURCETYPE_SCENE) << ")";
					obs_property_list_add_string(p, sstr.str().c_str(), name.c_str());
					return false;
				},
