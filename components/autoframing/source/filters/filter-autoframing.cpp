...
const char* streamfx::filter::autoframing::cstring(tracking_provider provider)
{
	...
	case tracking_provider::NVIDIA_FACEDETECTION:
		return obs_module_text(ST_I18N_ADVANCED_PROVIDER_NVIDIA_FACEDETECTION);
	default:
...
void streamfx::filter::autoframing::autoframing_instance::switch_provider(tracking_provider provider)
{
	...
	_provider_task = streamfx::util::threadpool::threadpool::instance()->push(std::bind(&autoframing_instance::task_switch_provider, this, std::placeholders::_1), spd);
}
...
obs_properties_t* autoframing_factory::get_properties2(autoframing_instance* data)
{
...

	{
		...
		obs_properties_add_group(pr, ST_I18N_TRACKING, obs_module_text(ST_I18N_TRACKING), OBS_GROUP_NORMAL, prg);

		{
			auto p = obs_properties_add_list(prg, ST_KEY_TRACKING_MODE, obs_module_text(ST_I18N_TRACKING_MODE), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
			obs_property_set_modified_callback(p, modified_provider);
			obs_property_list_add_int(p, obs_module_text(ST_I18N_FRAMING_MODE_SOLO), static_cast<int64_t>(tracking_mode::SOLO));
			obs_property_list_add_int(p, obs_module_text(ST_I18N_FRAMING_MODE_GROUP), static_cast<int64_t>(tracking_mode::GROUP));
		}

		{
			auto p = obs_properties_add_text(prg, ST_KEY_TRACKING_FREQUENCY, obs_module_text(ST_I18N_TRACKING_FREQUENCY), OBS_TEXT_DEFAULT);
		}
	}

	{
		auto prg = obs_properties_create();
		obs_properties_add_group(pr, ST_I18N_MOTION, obs_module_text(ST_I18N_MOTION), OBS_GROUP_NORMAL, prg);

		{
			auto p = obs_properties_add_float_slider(prg, ST_KEY_MOTION_SMOOTHING, obs_module_text(ST_I18N_MOTION_SMOOTHING), 0.0, 100.0, 0.01);
			obs_property_float_set_suffix(p, " %");
		}

		{
			auto p = obs_properties_add_float_slider(prg, ST_KEY_MOTION_PREDICTION, obs_module_text(ST_I18N_MOTION_PREDICTION), 0.0, 500.0, 0.01);
			obs_property_float_set_suffix(p, " %");
		}
	}
	...
	{
		auto prg = obs_properties_create();
		obs_properties_add_group(pr, ST_I18N_FRAMING, obs_module_text(ST_I18N_FRAMING), OBS_GROUP_NORMAL, prg);

		{
			auto p = obs_properties_add_float_slider(prg, ST_KEY_FRAMING_STABILITY, obs_module_text(ST_I18N_FRAMING_STABILITY), 0.0, 100.0, 0.01);
			obs_property_float_set_suffix(p, " %");
		}

		{
			auto prg2 = obs_properties_create();
			obs_properties_add_group(prg, ST_KEY_FRAMING_PADDING, obs_module_text(ST_I18N_FRAMING_PADDING), OBS_GROUP_NORMAL, prg2);

			{
				auto p = obs_properties_add_text(prg2, ST_KEY_FRAMING_PADDING ".X", "X", OBS_TEXT_DEFAULT);
			}
			{
				auto p = obs_properties_add_text(prg2, ST_KEY_FRAMING_PADDING ".Y", "Y", OBS_TEXT_DEFAULT);
			}
		}

		{
			auto prg2 = obs_properties_create();
			obs_properties_add_group(prg, ST_KEY_FRAMING_OFFSET, obs_module_text(ST_I18N_FRAMING_OFFSET), OBS_GROUP_NORMAL, prg2);

			{
				auto p = obs_properties_add_text(prg2, ST_KEY_FRAMING_OFFSET ".X", "X", OBS_TEXT_DEFAULT);
			}
			{
				auto p = obs_properties_add_text(prg2, ST_KEY_FRAMING_OFFSET ".Y", "Y", OBS_TEXT_DEFAULT);
			}
		}

		{
			auto p = obs_properties_add_list(prg, ST_KEY_FRAMING_ASPECTRATIO, obs_module_text(ST_I18N_FRAMING_ASPECTRATIO), OBS_COMBO_TYPE_EDITABLE, OBS_COMBO_FORMAT_STRING);
	...
	{ // Advanced Settings
		...
		{
			auto p = obs_properties_add_list(prg, ST_KEY_ADVANCED_PROVIDER, obs_module_text(ST_I18N_ADVANCED_PROVIDER), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
			obs_property_set_modified_callback(p, modified_provider);
			obs_property_list_add_int(p, obs_module_text(S_STATE_AUTOMATIC), static_cast<int64_t>(tracking_provider::AUTOMATIC));
#ifdef ENABLE_NVIDIA
			obs_property_list_add_int(p, obs_module_text(ST_I18N_ADVANCED_PROVIDER_NVIDIA_FACEDETECTION), static_cast<int64_t>(tracking_provider::NVIDIA_FACEDETECTION));
#endif
		}

		obs_properties_add_bool(prg, "Debug", "Debug");
	}
