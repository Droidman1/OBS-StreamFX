...
streamfx::gfx::mipmapper::mipmapper() : _gfx_util(::streamfx::gfx::util::get())
{
...

	{
		...
		try {
			_effect = streamfx::obs::gs::effect(file);
		} catch (const std::exception& ex) {
			DLOG_ERROR("Error loading '%s': %s", file.generic_string().c_str(), ex.what());
		...
		if (!separable_technique) {
			try {
				...
				if (mip == 1) {
					...
					_effect.get_parameter("image").set_texture(*rt1, enable_srgb);
				}
				_effect.get_parameter("imageTexel").set_float2(iwidth, iheight);
				_effect.get_parameter("separableTexel").set_float2(iwidth, iheight);
				while (gs_effect_loop(_effect, technique.c_str())) {
					_gfx_util->draw_fullscreen_triangle();
				}
			} catch (...) {
			}
		} else {
			for (size_t pass = 0; gs_effect_loop(_effect, technique.c_str()); pass++) {
				// Flow:
				// Mip-Level 1:
				// - Pass 0:
				//   - Swap rt0, rt2
				...
				if (mip == 1) {
					...
					_effect.get_parameter("image").set_texture(*rt2, enable_srgb);
				}
				_effect.get_parameter("imageTexel").set_float2(iwidth, iheight);
				if ((pass % 2) == 0) {
...
#ifdef _WIN32
		if (gs_get_device_type() == GS_DEVICE_DIRECT3D_11) {
			d3d_copy_subregion(d3dinfo, *rt0, static_cast<uint32_t>(mip), cwidth, cheight);
		}
#endif
		if (gs_get_device_type() == GS_DEVICE_OPENGL) {
			opengl_copy_subregion(oglinfo, *rt0, static_cast<uint32_t>(mip), cwidth, cheight);
		}
