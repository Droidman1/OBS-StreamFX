...
{
	{
		...
		try {
			...
			D_LOG_ERROR("Failed to load '%s'.", file.generic_string().c_str());
		}
	}
}
...
void streamfx::gfx::util::draw_point(float x, float y, uint32_t color)
{
	...
	if (!_point_vb) {
		_point_vb = std::make_shared<obs::gs::vertexbuffer>(uint32_t{1}, uint8_t{1});
	}
	...
	while (gs_effect_loop(*_effect, "Color")) {
...
void streamfx::gfx::util::draw_line(float x, float y, float x2, float y2, uint32_t color /*= 0xFFFFFFFF*/)
{
	...
	if (!_line_vb) {
		_line_vb = std::make_shared<obs::gs::vertexbuffer>(uint32_t{2}, uint8_t{1});
	}
...
void streamfx::gfx::util::draw_arrow(float x, float y, float x2, float y2, float w /*= 0.*/, uint32_t color /*= 0xFFFFFFFF*/)
{
	...
	if (!_arrow_vb) {
		_arrow_vb = std::make_shared<obs::gs::vertexbuffer>(uint32_t{5}, uint8_t{1});
	}
...
void streamfx::gfx::util::draw_rectangle(float x, float y, float w, float h, bool frame, uint32_t color /*= 0xFFFFFFFF*/)
{
	...
	if (!_quad_vb) {
		_quad_vb = std::make_shared<obs::gs::vertexbuffer>(uint32_t{5}, uint8_t{1});
	}
...
{
	if (!_fstri_vb) {
		_fstri_vb = std::make_shared<streamfx::obs::gs::vertexbuffer>(uint32_t(3), uint8_t(1));
		{
