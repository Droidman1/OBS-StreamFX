...
streamfx::util::threadpool::task::task(task_callback_t callback, task_data_t data) : _callback(callback), _data(data), _state(worker_state::DEFAULT) {}
...
void streamfx::util::threadpool::task::run()
{
	...
	if (_state == worker_state::DEFAULT) {
		try {
			_callback(_data);
			_state = worker_state::COMPLETED;
		} catch (const std::exception& ex) {
			D_LOG_ERROR("Unhandled exception in Task: %s.", ex.what());
			_state = worker_state::FAILED;
		} catch (...) {
...
{
	std::lock_guard<std::mutex> lg(_lock);
	if (_state == worker_state::DEFAULT) { // Don't override an already existing other state.
		_state = worker_state::CANCELLED;
		_status_changed.notify_all();
	}
}
...
{
	return _state == worker_state::CANCELLED;
}

bool streamfx::util::threadpool::task::is_completed()
{
	return _state == worker_state::COMPLETED;
}

bool streamfx::util::threadpool::task::has_failed()
{
	return _state == worker_state::FAILED;
}
...
void streamfx::util::threadpool::task::wait()
{
	...
	if (_state == worker_state::DEFAULT) {
		_status_changed.wait(ul, [this]() { return _state != worker_state::DEFAULT; });
	}
}
...
bool streamfx::util::threadpool::task::is_processing()
{
	return _state == worker_state::DEFAULT;
}
...
streamfx::util::threadpool::threadpool::threadpool(size_t minimum, size_t maximum) : _limits{minimum, maximum}, _worker_count(0)
{
...
std::shared_ptr<streamfx::util::threadpool::task> streamfx::util::threadpool::threadpool::push(task_callback_t callback, task_data_t data /*= nullptr*/)
{
	...
	// Actually notify the threads that there's work.
	_tasks_cv.notify_all();
