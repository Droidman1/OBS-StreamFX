...
namespace streamfx::obs {
	class source {
		 ...
		 */
		FORCE_INLINE source(std::string_view name)
			: _is_owner(true)
		{
		 ...
		 * Attention: May fail.
		 */
		FORCE_INLINE source(std::string_view id, std::string_view name, obs_data_t* settings)
			: _is_owner(true)
		{
		...
		{
			const char* name = obs_source_get_name(_ref);
		...
		{
			return {obs_filter_get_parent(_ref), false, false};
		};
		...
		{
			return {obs_filter_get_target(_ref), false, false};
		};
		...
		/** 
		 *
		 * EXPORT bool obs_source_process_filter_begin_with_color_space(obs_source_t *filter, enum gs_color_format format, enum gs_color_space space, enum obs_allow_direct_render allow_direct);
		 */
		FORCE_INLINE bool process_filter_begin(gs_color_format format, gs_color_space space, obs_allow_direct_render allow_direct)
		{
			return obs_source_process_filter_begin_with_color_space(_ref, format, space, allow_direct);
		};
		...
		{
#ifdef _DEBUG
			blog(LOG_DEBUG, "obs_source_inc_active on %s (UUID: %s)", obs_source_get_name(_ref), obs_source_get_uuid(_ref));
#endif
		...
		{
#ifdef _DEBUG
			blog(LOG_DEBUG, "obs_source_dec_active on %s (UUID: %s)", obs_source_get_name(_ref), obs_source_get_uuid(_ref));
#endif
		...
		{
#ifdef _DEBUG
			blog(LOG_DEBUG, "obs_source_inc_showing on %s (UUID: %s)", obs_source_get_name(_ref), obs_source_get_uuid(_ref));
#endif
		...
		{
#ifdef _DEBUG
			blog(LOG_DEBUG, "obs_source_dec_showing on %s (UUID: %s)", obs_source_get_name(_ref), obs_source_get_uuid(_ref));
#endif
		...
		/**
		 * 
		 * EXPORT enum gs_color_space obs_source_get_color_space(obs_source_t *source, size_t count, const enum gs_color_space *preferred_spaces);
		 */
		gs_color_space color_space(size_t count, gs_color_space preferred_spaces[])
		{
			return obs_source_get_color_space(_ref, count, preferred_spaces);
		}

		public /* Children */:
		/** Enumerate active child sources provided by this source.
		 */
		void enumerate_active_sources(obs_source_enum_proc_t callback, void* param)
		{
			return obs_source_enum_active_sources(_ref, callback, param);
		}

		/** Recursively enumerate active child sources.
		 */
		void enumerate_active_tree(obs_source_enum_proc_t callback, void* param)
		{
			return obs_source_enum_active_tree(_ref, callback, param);
		}

		/** Recursively enumerate child sources.
		 */
		void enumerate_full_tree(obs_source_enum_proc_t callback, void* param)
		{
			return obs_source_enum_full_tree(_ref, callback, param);
		}

		/** Enumerate direct child sources.
		 *
		 * This is not standard libOBS and only valid on StreamFX sources. It is necessary to bypass some nonsensical lockups.
		 */
		void enumerate_children(obs_source_enum_proc_t callback, void* param)
		{
			// Allocate a calldata object on stack, without the needless overhead.
			calldata_t cd;
			uint8_t    stack[256];
			calldata_init_fixed(&cd, stack, 256);
			calldata_set_ptr(&cd, "callback", (void*)callback);
			calldata_set_ptr(&cd, "param", (void*)param);

			// Call the procedure.
			auto ph = obs_source_get_proc_handler(_ref);
			proc_handler_call(ph, "enumerate_children", &cd);
		}
		...
		FORCE_INLINE bool operator==(obs_source_t* rhs) const
		{
			return _ref == rhs;
		};
	};
