...
// ToDo: Add optimization to source framework for filters that automatically handles input/output.
...
namespace streamfx::obs {
	...
	class source_factory {
		...
		void register_proxy(std::string_view name)
		{
			...
			proxy->output_flags |= OBS_SOURCE_DEPRECATED | OBS_SOURCE_CAP_DISABLED;
		...
		static void _filter_add(void* data, obs_source_t* source) noexcept
		{
			auto self = reinterpret_cast<_instance*>(data);

			try {
				if (data)
					self->filter_add(source);

				if (self->get().type() == OBS_SOURCE_TYPE_FILTER) { // Propagate the proper filter_add signal to the filter itself.
					auto        sh = obs_source_get_signal_handler(*self->get());
					calldata_t* cd = calldata_create();
					calldata_set_ptr(cd, "source", source);
					calldata_set_ptr(cd, "filter", *self->get());
					signal_handler_signal(sh, "attach", cd);
					calldata_destroy(cd);
				}
			} catch (const std::exception& ex) {
				DLOG_ERROR("Unexpected exception in function '%s': %s.", __FUNCTION_NAME__, ex.what());
		...
		static void _filter_remove(void* data, obs_source_t* source) noexcept
		{
			...
			try {
				if (data)
					self->filter_remove(source);

				if (self->get().type() == OBS_SOURCE_TYPE_FILTER) { // Propagate the proper filter_remove signal to the filter itself.
					auto        sh = obs_source_get_signal_handler(*self->get());
					calldata_t* cd = calldata_create();
					calldata_set_ptr(cd, "source", source);
					calldata_set_ptr(cd, "filter", *self->get());
					signal_handler_signal(sh, "detach", cd);
					calldata_destroy(cd);
				}
			} catch (const std::exception& ex) {
				DLOG_ERROR("Unexpected exception in function '%s': %s.", __FUNCTION_NAME__, ex.what());
	...
	class source_instance {
		...
		public:
		source_instance(obs_data_t* settings, obs_source_t* source)
			: _self(source, false, false)
		{
			// Provide a way to bypass libOBS when it comes to enumerating child sources. We need this due to a few problems that otherwise ruin the whole experience.

			{ // Procedures
				auto ph = obs_source_get_proc_handler(source);

				// Add enumeration procedure for other components to attach to.
				proc_handler_add(
					ph, "void enumerate_children(ptr callback, ptr param)",
					[](void* ptr, calldata_t* cd) {
						obs_source_enum_proc_t callback = nullptr;
						void*                  param    = nullptr;
						if (!calldata_get_ptr(cd, "callback", &callback) || !calldata_get_ptr(cd, "param", &param)) {
							return;
						}

						reinterpret_cast<source_instance*>(ptr)->enum_children(callback, param);

						// Call the signal
						auto sh = obs_source_get_signal_handler(reinterpret_cast<source_instance*>(ptr)->_self);
						signal_handler_signal(sh, "enumerate_children", cd);
					},
					this);
			}
			...
			{ // Signals
				...
				// Add enumeration signal for other components to attach to.
				signal_handler_add(sh, "void enumerate_children(ptr callback, ptr param)");

				// Attach / Detach from Parent Source
				signal_handler_add(sh, "void attach(ptr source, ptr filter)");
				signal_handler_add(sh, "void detach(ptr source, ptr filter)");
			}
		}
		...
		virtual void filter_add(obs_source_t* source) {}
		...
		virtual void enum_children(obs_source_enum_proc_t enum_callback, void* param) {}

		public /* Instance > Configuration */:
		virtual void load(obs_data_t* settings) {}

		/** Called prior to load() to allow for migration of data to a newer format.
		 */
