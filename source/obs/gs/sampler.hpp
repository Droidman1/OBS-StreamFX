...
struct std::hash<gs_sampler_info> {
	...
	{
		auto hash = std::hash<uint8_t>{}(static_cast<uint8_t>(si.address_u));
		hash ^= std::hash<uint8_t>{}(static_cast<uint8_t>(si.address_v));
		hash ^= std::hash<uint8_t>{}(static_cast<uint8_t>(si.address_w));
		hash ^= std::hash<uint32_t>{}(static_cast<uint32_t>(si.border_color));
		hash ^= std::hash<uint8_t>{}(static_cast<uint8_t>(si.filter));
		hash ^= std::hash<uint8_t>{}(static_cast<uint8_t>(si.max_anisotropy));
		return hash;
	}
};
...
namespace streamfx::obs::gs {
	class sampler {
		...
		typedef streamfx::util::multipool<streamfx::obs::gs::sampler::pool, streamfx::obs::gs::sampler, 100, _pool_key_t> _pool_t;
