...
namespace streamfx::obs::gs {
	class effect_parameter {
		gs_eparam_t*                 _param; // Kept alive by _effect
		...
		public:
		effect_parameter();
		effect_parameter(nullptr_t);
		effect_parameter(gs_eparam_t* param, std::shared_ptr<gs_effect_t> effect);
		~effect_parameter();

		// Move Constructor & Assignment
		effect_parameter(streamfx::obs::gs::effect_parameter&&) noexcept;
		effect_parameter& operator=(streamfx::obs::gs::effect_parameter&&) noexcept;

		// Copy Constructor & Assignment
		effect_parameter(const streamfx::obs::gs::effect_parameter&) noexcept;
		effect_parameter& operator=(const streamfx::obs::gs::effect_parameter&) noexcept;
		...
		{
			return gs_effect_get_default_val_size(_param);
		}

		template<typename T>
		std::size_t get_default_value_size()
		{
			return gs_effect_get_default_val_size(_param) / sizeof(T);
		}
		...
		bool get_default_value(T v[], std::size_t len)
		{
			...
			if (T* ptr = reinterpret_cast<T*>(gs_effect_get_default_val(_param)); ptr != nullptr) {
		...
		{
			return gs_effect_get_val_size(_param);
		}

		template<typename T>
		std::size_t get_value_size()
		{
			return gs_effect_get_val_size(_param) / sizeof(T);
		}
		...
		bool get_value(T v[], std::size_t len)
		{
			...
			if (T* ptr = reinterpret_cast<T*>(gs_effect_get_val(_param)); ptr != nullptr) {
				for (std::size_t idx = 0; idx < len; idx++) {
					v[idx] = *(ptr + idx);
				}
		...
		{
			gs_effect_set_val(_param, v, sizeof(T) * len);
			return true;
		}
		...
		public /* Comparison */:
		bool operator==(const streamfx::obs::gs::effect_parameter&) const;

		bool operator==(gs_eparam_t*) const;
