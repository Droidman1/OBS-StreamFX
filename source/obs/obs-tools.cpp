...
void __source_find_source_enumerate(obs_source_t* haystack, __sfs_data* cbd)
{
	...
	if (obs_source_get_type(haystack) == OBS_SOURCE_TYPE_FILTER) {
		haystack = obs_filter_get_parent(haystack);
	}
	...
	streamfx::obs::source(haystack, false, false)
		.enumerate_children(
			[](obs_source_t* parent, obs_source_t* child, void* param) {
				try {
					__source_find_source_enumerate(child, reinterpret_cast<__sfs_data*>(param));
	...
	if (obs_source_get_type(haystack) == OBS_SOURCE_TYPE_SCENE) {
...
bool streamfx::obs::tools::source_find_source(::streamfx::obs::source haystack, ::streamfx::obs::weak_source needle)
{
	__sfs_data cbd = {};
	try {
		__source_find_source_enumerate(*haystack, &cbd);
	} catch (...) {
	}
