...
lsb_distributor=$(lsb_release -si 2>/dev/null)
lsb_description=$(lsb_release -sd 2>/dev/null)
lsb_release=$(lsb_release -sr 2>/dev/null)
platform=$(uname)
suffix=""
echo "$(uname) $OSTYPE"
...
function task_libobs {
	if [[ "$platform" == "linux" ]]; then
		...
		if [[ ("$lsb_distributor" == "Ubuntu") ]]; then
			...
			if [[ ("$lsb_release" == "24"*) ]]; then
				...
				echo "This version of Ubuntu is no longer supported by libOBS."
				return 1
			fi
		else
			echo "Unknown Unix Operating System: $lsb_distributor $lsb_release"
			return 1
		fi
		group_end
	elif [[ "$platform" == "mac" ]]; then
		group_start "Install Pre-requisites"
		brew install git; check_err
		brew install cmake; check_err
		group_end
	fi
	...
	cmake \
		...
		--target obs-frontend-api; check_err
	group_end
	...
	cmake \
		...
		--config RelWithDebInfo \
		--component Development; check_err
	group_end
}
...
while [[ "$1" != "" ]]; do
	if [[ $(type -t "task_$1") == "function" ]]; then
		...
		# Stop here and leave, all further parameters are to be passed through directly.
		shift
		break
	else
		args_unused+=("$1")
